#!/usr/bin/perl
#
#  $Source: /opt/cvsroot/projects/Spamity/lib/Spamity/Preference/amavisdnew.pm,v $
#  $Name:  $
#
...
sub getPolicy
{
	...
	if (defined $id) {
	    ...
	    $stmt = sprintf('select u.id as user_id, p.* from %s as u left join %s as p on u.policy_id = p.id where lower(u.email) = ?',
