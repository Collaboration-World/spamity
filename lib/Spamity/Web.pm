#!/usr/bin/perl
#
#  $Source: /opt/cvsroot/projects/Spamity/lib/Spamity/Web.pm,v $
#  $Name:  $
#
...
BEGIN {
    ...
    @Spamity::Web::EXPORT_OK = qw(cache @FILTERS);
}
use vars qw(@FILTERS);
...
sub getGraphByCount
{
    ...
    $graph->set(
		x_label      => '',
		x_label_position => 0,
		y_label      => &translate('number of rejected messages') . ((defined $date)?' '.&translate('since')." $date":""),
		y_min_value  => 0,
...
sub getGraphByUser
{
    ...
    $graph->set(
		x_label      => '',
		y_label      => &translate('number of rejected messages') . ((keys(%stats) > 0)?' '.&translate('since')." $date":""),
		y_min_value  => 0,
